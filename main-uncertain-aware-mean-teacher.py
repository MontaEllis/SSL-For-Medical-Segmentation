...
def parse_training_args(parser):
    ...
    parser.add_argument('-o', '--output_dir', type=str, default='logs-0505', required=False, help='Directory to save checkpoints')
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i, batch in enumerate(train_loader):
            ...
            noise = torch.clamp(torch.randn_like(
                unlabel_img) * 0.1, -0.2, 0.2)
            ema_inputs = unlabel_img + noise

            outputs_soft = torch.sigmoid(outputs)
            with torch.no_grad():
                ema_output = model_ema(ema_inputs)
                # ema_output_soft = torch.sigmoid(ema_output)
            
            T = 8
            _, _, d, w, h = unlabel_img.shape
            volume_batch_r = unlabel_img.repeat(2, 1, 1, 1, 1)
            ...
            consistency_dist = softmax_mse_loss(
                outputs, ema_output)  # (batch, 2, 112,112,80)
            threshold = (0.75+0.25*sigmoid_rampup(epoch,
                                                        epochs))*np.log(2)
