...
def parse_training_args(parser):
    ...
    parser.add_argument('-o', '--output_dir', type=str, default='logs-0506', required=False, help='Directory to save checkpoints')
...
def train():
    ...
    if hp.mode == '2d':
        ...
        model_2 = UNETR(img_shape=(hp.crop_or_pad_size,hp.crop_or_pad_size,hp.crop_or_pad_size), input_dim=hp.in_class, output_dim=hp.out_class)
    ...
    optimizer_2 = torch.optim.Adam(model_2.parameters(), lr=args.init_lr)

    # scheduler = ReduceLROnPlateau(optimizer, 'min',factor=0.5, patience=20, verbose=True)
    scheduler = StepLR(optimizer, step_size=30, gamma=0.8)
    scheduler_2 = StepLR(optimizer_2, step_size=30, gamma=0.8)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i, batch in enumerate(train_loader):
            ...
            outputs_2_un = model(unlabel_img)
            outputs_soft_2_un = torch.sigmoid(outputs_2_un)

            consistency_weight = math.exp(-0.1*(20-epoch)**1.5)

            loss1 = 0.5 * (ce_loss(outputs,
                                   y) )
            loss2 = 0.5 * (ce_loss(outputs_2,
                                   y) )
            ...
            # temp = pseudo_outputs1
            # temp[temp>0.5] = 1
            # temp[temp<0.5] = 0
            # pseudo_outputs1 = temp

            pseudo_outputs2 = (outputs_soft_2_un.detach()>0.5).float()
            # temp_ = pseudo_outputs2
            # temp_[temp_>0.5] = 1
            # temp_[temp_<0.5] = 0
            # pseudo_outputs2 = temp_


            pseudo_supervision1 = ce_loss(outputs_un, pseudo_outputs1)
            pseudo_supervision2 = ce_loss(outputs_2_un, pseudo_outputs2)
