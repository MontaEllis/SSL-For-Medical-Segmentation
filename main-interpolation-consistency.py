...
def parse_training_args(parser):
    ...
    parser.add_argument('-o', '--output_dir', type=str, default='logs-0504', required=False, help='Directory to save checkpoints')
    ...
    training.add_argument('--batch', type=int, default=2, help='batch-size')     #12
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i, batch in enumerate(train_loader):
            ...
            ict_mix_factors = np.random.beta(
                0.2, 0.2, size=(args.batch//2, 1, 1, 1, 1))
            ict_mix_factors = torch.tensor(
                ict_mix_factors, dtype=torch.float).cuda()
            unlabeled_volume_batch_0 = unlabel_img[0:1, ...]
            unlabeled_volume_batch_1 = unlabel_img[1:2, ...]
            ...
            consistency_loss = torch.mean(
                (outputs_soft - batch_pred_mixed)**2)
