...
def parse_training_args(parser):
    ...
    parser.add_argument('-o', '--output_dir', type=str, default='logs_0506', required=False, help='Directory to save checkpoints')
...
def train():
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i, batch in enumerate(train_loader):
            ...
            rotated_unlabeled_volume_batch = torch.rot90(unlabel_img, rot_times, [2,3])

            unlabeled_rot_outputs = model(rotated_unlabeled_volume_batch)
            unlabeled_rot_outputs_soft = torch.sigmoid(unlabeled_rot_outputs)
            ...
            consistency_loss = 0.5 * (torch.mean((unlabeled_rot_outputs_soft.detach() - torch.rot90(outputs_soft, rot_times, [2,3]))**2) + torch.mean((unlabeled_rot_outputs_soft - torch.rot90(outputs_soft.detach(), rot_times, [2,3]))**2))
