...
from data_function import My_train
...
def train():
    ...
    if hp.mode == '2d':
        ...
        # model = UNet3D(in_channels=hp.in_class, out_channels=hp.out_class, init_features=32)#, base_n_filter=2)  #2

        # model_ema = UNet3D(in_channels=hp.in_class, out_channels=hp.out_class, init_features=32)
    ...
    train_dataset = My_train(train_dataset.queue_dataset,train_dataset.queue_dataset_2)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i, (batch1,batch2) in enumerate(train_loader):
            ...
            x = batch1['source']['data']
            x_aux = batch2['source']['data']
            y = batch1['label']['data']
            y_aux = batch2['label']['data']

            x = x.type(torch.FloatTensor).cuda()
            x_aux = x_aux.type(torch.FloatTensor).cuda()
            y = y.type(torch.FloatTensor).cuda()
            y_aux = y_aux.type(torch.FloatTensor).cuda()



            #################
            outputs = model(x)
            outputs_aux = model(x_aux)
            outputs_un = model(unlabel_img)
            outputs_un_aux = model(unlabel_img_unaug)

            loss_ce = ce_loss(outputs,
                              y) + ce_loss(outputs_aux,
                              y_aux)            
            # loss_ce = abs(weight_loss(outputs,
            #                   y) + weight_loss(outputs_aux,
            #                   y_aux))
            # loss_dice = dice_loss(
            #     outputs_soft, y)
            supervised_loss = 0.5 * (loss_ce)
            consistency_weight = 0.5*math.exp(-0.1*(20-epoch)**1.5)


            # consistency_loss_1 = torch.mean(
            #     (outputs - outputs_aux)**2)
            # consistency_loss_2 = torch.mean(
            #     (outputs_un - outputs_un_aux)**2)
            consistency_loss_1 = abs(weight_loss(outputs, outputs_aux))
            consistency_loss_2 = abs(weight_loss(outputs_un, outputs_un_aux))

            loss = supervised_loss + consistency_weight * (consistency_loss_1 + consistency_loss_2)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            #################
        ...
        if epoch % args.epochs_per_checkpoint == 0:
            ...
            with torch.no_grad():
                ...
                affine = batch1['source']['affine'][0].numpy()
...
def test():
    ...
    if hp.mode == '2d':
        ...
        # from models.three_d.unet3d import UNet3D
        # model = UNet3D(in_channels=hp.in_class, out_channels=hp.out_class, init_features=32)#, base_n_filter=2)
